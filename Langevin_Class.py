...
class LangevinSimulator:
    ...
    def msd(self, traj, time_end=1/4, msd_nbpt = 2000,print_time=False):
        traj = np.unwrap(traj)
        t0 = time.time()
    ...
    def mean_msd_and_time(self, trajs, n_jobs=5, time_end=1/4, msd_nbpt = 2000, nb_traj = None,t = int(1e6)):
        """
        ...
        for i in range(len(trajs[:nb_traj])):
            msd_matrix.append(self.msd(trajs[i], time_end=1/4, msd_nbpt = 2000))
        ...
        matrix_traj = [trajs[i][t]  for i in range(len(trajs))]
    ...
    def full_auto_trajs_mean_msd(self,ampl_range = np.arange(8),nb_traj = 30, npts = int(1e7) ):
        """
        ...
        for A in ampl_range:
            trajs = self.chunkify_traj(repetitions=nb_traj, n_jobs=5, npts = npts, x0s = x0s, Amplitude = A, torque = 0)
            trajectories_box.append(trajs)
            time_axis,mean_msd = self.mean_msd_and_time(trajs, time_end=1/4, msd_nbpt = 2000, nb_traj = nb_traj)
    ...
    def asdefaveri_msd(self,ampl_range):
        
        a = 2*np.pi/self.frequency
        D = self.D          
        D_eff = self.lifson_jackson_noforce
        time_axis,mean_msd_box,trajectories_box = self.full_auto_trajs_mean_msd(ampl_range = ampl_range,nb_traj = 10, npts = int(1e8) )
        
        def defaveri_stationary(A,U):
        ...
        def boltz_exp(A,x,U):
            Ux = int(np.round(x*len(U)/(2*np.pi)) - 1)
            return np.exp(-Ux)
        ...
        for i in range(len(ampl_range)):
            ...
            print('D_eff = ',D_eff)
            stationary_state = defaveri_stationary(ampl_range[i],U)
            results.append(absciss,norm_msd,D_fit,D_eff,shift,stationary_state,a)
            plt.scatter(time_axis,mean_msd_box[i],label=f'{ampl_range[i]} KT')
            plt.plot(time_axis,2*time_axis[i]*D_eff)
            plt.legend()
    """
