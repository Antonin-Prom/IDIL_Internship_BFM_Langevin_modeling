...
class DiffusionSimulation2:
    ...
    def compute_autocorrelation(self,histograms):
        num_histograms = len(histograms)
        autocorrelations = []
        for i in range(num_histograms):
            for j in range(i+1, num_histograms):
                # Compute autocorrelation using numpy.correlate
                autocorr = np.correlate(histograms[i], histograms[j], mode='full')
                autocorrelations.append(autocorr)
        return np.array(autocorrelations)
            
    def manual_hist_array(traj,N_bin):
        d_angle = 2*np.pi/N_bin
        sort_traj = np.sort(np.mod(traj,2*np.pi))
        bins = np.zeros(N_bin)
        for j,angle in zip(range(N_bin),sort_traj):
            if angle < (j+1)*d_angle:
                bins[j] += 1
            else:
                continue
        return bins
