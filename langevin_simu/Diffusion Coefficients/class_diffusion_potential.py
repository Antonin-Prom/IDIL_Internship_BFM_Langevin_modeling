...
class DiffusionSimulation:
    def __init__(self,frequency = 26):
        # Constants
        self.T_K = 300
    ...
    def lifson_jackson(self, amplitude):
    ...
    def simulate(self, amplitude, nb_traj, traj_length):
        ...
        lifson_jackson_diffusion_coefficient = self.lifson_jackson(amplitude)
        bessel_diffusion_coefficient = self.Bessel(amplitude)
        return simulated_diffusion, lifson_jackson_diffusion_coefficient, bessel_diffusion_coefficient
    
    def Lifson_Jack_Simu(self, amplitude_array = np.linspace(0, 1, 1000)):
        lifson_jackson_diffusion_coefficient_array = np.zeros(len(amplitude_array))
        counter = 0
        for amplitude in amplitude_array:
            lifson_jackson_diffusion_coefficient_array[counter] = self.lifson_jackson(amplitude)
