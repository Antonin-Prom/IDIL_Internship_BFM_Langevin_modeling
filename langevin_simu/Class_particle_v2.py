...
class DiffusionSimulation2:
    ...
    def calculate_msd(self, traj, time_end=1/4, msd_nbpt = 2000,n_jobs=5):
        max_lagtime = int(len(traj) * time_end)
        total_lag_time = np.unique([int(lag) for lag in np.logspace(0, int(np.log10(max_lagtime)), msd_nbpt)])
    ...
    def mean_msd(self, function, trajs):
        mean_msd = []
        for traj in trajs:
            mean_msd.append(function(traj=traj))
        std_msd = np.std(mean_msd)
    ...
    def compare_msd(self, N):
        ...
        t0 = time.time()
        msd,std = self.mean_msd(self.mean_square_displacement_list, trajs)
        print(f'run_msd_regular(): done in {time.time() - t0:.1f} s')
    
        print(f'run_msd_calculate(): Parallel working...')
        t0 = time.time()
        msd_calculate,std_calculate = self.mean_msd(self.calculate_msd, trajs)
        print(f'run_msd_calculate(): Parallel done in {time.time() - t0:.1f} s')
    
        print(f'run_msd_chunk(): Parallel working...')
        t0 = time.time()
        msd_chunk,std_chunk = self.mean_msd(self.run_parallel_msd_chunk_log, trajs)
        print(f'run_msd_chunk(): Chunk done in {time.time() - t0:.1f} s')
        
        return [msd,msd_calculate, msd_chunk],[std,std_calculate,std_chunk]
...
[msd,msd_calculate,msd_chunk],[std,std_calculate,std_chunk] = part.compare_msd(N)
t = np.unique([int(lag) for lag in np.logspace(0, int(np.log10(N/4)), 2000)])*1e-5
 
fig, ax = plt.subplots(figsize = (9, 6))
ax.plot(t, msd_calculate, label = 'mean MSD, free_diffusion')
#ax.scatter(t, msd_chunk, s=1, label = 'MSD mean 2nd method for 0kT')
ax.fill_between(t, msd_calculate - std_calculate, msd_calculate + std_calculate, color='gray', alpha=0.3)
