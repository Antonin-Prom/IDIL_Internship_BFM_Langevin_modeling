...
class TiltedPeriodicDiffusion():
    ...
    def make_trace(self, U, npts=10000, plots=False):
        ...
        # initial condition:
        x = np.zeros(npts) 
        # numba make trace, x is wrapped in 0,2pi:
        x_wrap = _make_trace(x, npts, dUdx, self.dt, self.gamma, thermnoise)
        # wrap x in (0,2pi):
        x_unwrap = np.unwrap(x)
        print(f'make_trace(): Done in {time.time() - t0} s.')
        if np.any(np.nonzero(np.isnan(x))):
            print('make_trace(): Warning, Nan in x(t)')
        if plots:
            t0 = time.time()
