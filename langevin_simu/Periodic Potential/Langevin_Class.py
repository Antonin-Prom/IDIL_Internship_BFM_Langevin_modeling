...
class LangevinSimulator:
    ...
    def normalised_loglog_msd_fit(self,ampl_range,npts):
        ...
        np.save(f'To_plot_t,{npts}npts_msd_0_torque_{self.torque}kT_dt={self.dt}',[time_box,mean_msd_box])
    ...
    def fit_and_plot_msd(self, time_axis_boxbox, mean_msd_boxbox, ampl_range, torque_range):
        ...
        for j in range(len(torque_range)):
            ...
            for i in range(len(ampl_range)):
                ...
                if j==0:
                    ...
                    """
                    popt, pcov = scipy.optimize.curve_fit(linear_D, time_axis_box[i][window_indices], mean_msd_box[i][window_indices])
                    D_fit = popt[0]
                    print(f'For A = {ampl_range[i]} and torque = 0 D_fit  = ', D_fit)
                    print('D_eff = ', D_eff)
                    #plt.plot(absciss[int((len(absciss)/frac)):], linear_D(time_axis_box[i],D_eff)[int((len(absciss)/frac)):]/(a*a),  linewidth=1, linestyle='--', color='r')
                    stationary_state = defaveri_stationary(ampl_range[i])
                    #plt.plot(absciss, norm_msd, label=f'A={ampl_range[i]} KT, T={torque_range[j]}', linewidth=1, color=colors[i])
                ...
                #plt.plot(absciss[int((len(absciss)/frac)):], parabolic_msd(time_axis_box[i],D_eff)[int((len(absciss)/frac)):]/(a*a),  linewidth=1, linestyle='--', color='royalblue')                
        ...
        # Save the figure
        plt.savefig('msd_plot_bead.png', dpi=300)
        plt.show()
...
t0,msd0 = np.load('To_plot_t,msd_0_torque_0kT_dt=0.0001.npy')
t10,msd10 = np.load('To_plot_t,msd_0_torque_10kT_dt=0.0001.npy')
t20,msd20 = np.load('To_plot_t,msd_0_torque_20kT_dt=0.0001.npy')
