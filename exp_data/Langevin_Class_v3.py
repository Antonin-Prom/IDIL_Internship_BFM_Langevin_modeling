...
class LangevinSimulator:
    ...
    def full_integrand(self,x,amplitude,sign):
        return np.exp(sign*self.analytical_potential(x, amplitude))
    
    def factor1(self, amplitude):  
        result, _ = quad(self.integrand1, -np.pi / self.frequency, np.pi / self.frequency, args=(amplitude))
        return result
    
    def full_factor(self,amplitude,sign):
        L = 2 * np.pi / self.frequency
        result, _ = quad(self.full_integrand, -L/2, L/2, args=(amplitude,sign))
        return result    
    ...
    def tilt_LJ(self,amplitude):
        """
        Equation from :
        Biased diffusion in periodic potentials: Three
        types of force dependence of effective diffusivity
        and generalized Lifson-Jackson formula
        """
        L = 2 * np.pi / self.frequency
        F = self.torque/(2*np.pi) #KT already taken in acount            
        lifson_jackson2 = (self.D * L**2 ) / ((self.full_factor(amplitude,-1)) * (self.full_factor(amplitude,1))) * ((np.sinh(F*L/2))/(F*L/2))**2
    ...
    def D_eff_reimann(self,amplitude):
        ...
        def I_plus(x):
            result, _ = quad(self.full_integrand, x - a, x, args=(A,1))
            return (1 / self.D) * self.full_integrand(x, A, -1) * result
        
        def I_minus(x):
            result, _ = quad(self.full_integrand, x , x + a, args=(A,-1))
            return (1 / self.D) * self.full_integrand(x, A, 1) * result
